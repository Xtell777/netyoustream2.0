...
if (!isset($_SESSION['user_id'])) {
    // Se não estiver logado, salvar a URL da página atual para redirecionar após o login
...
<head>
    ...
    <title>Net You Stream - Envio de Arquivos</title>
...
.subscribe-button {
    ...
    font-size: 16px;
    cursor: pointer;
    margin-left: 10px; /* Adicione este estilo para ajustar a margem esquerda do botão */
}
...
/* Estilo para o container de envio de URL */
#url-upload {
    margin-top: 20px;
    padding: 15px;
...
#video_url:focus {
    border-color: #007bff; /* Cor de destaque para o campo ativo */
...
#upload-url-button:focus {
    outline: none;
    box-shadow: 0 0 5px rgba(0, 123, 255, 0.5);
}
/* Estilos gerais para a seção de vídeos enviados */
#uploadedVideos {
    margin: 40px auto;
    padding: 20px;
    max-width: 1200px;
...
/* Estilos para o contêiner dos vídeos */
#videoList {
    display: grid;
...
/* Estilos para cada item de vídeo */
.video-item {
    width: 100%;
    max-width: 300px;
    background-color: #fff;
...
/* Efeito de hover para cada vídeo */
.video-item:hover {
...
/* Estilos para o botão de interação */
.interaction-buttons {
    display: flex;
...
<body>
    ...
    <div id="upload-container">
        ...
        <div id="upload-form">
            <form id="uploadForm" action="processa_uploads.php" method="post" enctype="multipart/form-data">
                <label for="file"><i class="fas fa-file"></i> Escolha um arquivo:</label>
                <input type="file" id="file" name="file" accept="video/*, image/*" required>
                <label for="title"><i class="fas fa-font"></i> Nome do arquivo:</label>
                <input type="text" id="title" name="title" required>
                <label for="description"><i class="fas fa-comment"></i> Descrição do arquivo:</label>
                <textarea id="description" name="description"></textarea>
                <label for="category"><i class="fas fa-list-alt"></i> Categoria:</label>
        ...
        <!-- Novo campo para enviar URLs de vídeo -->
        <div id="url-upload">
            <h3>Ou envie uma URL de vídeo</h3>
            <label for="video_url"><i class="fas fa-link"></i> URL do vídeo:</label>
            <input type="url" id="video_url" name="video_url" placeholder="https://example.com/video" required>
            <button id="upload-url-button">Enviar URL</button>
        </div>

        <progress id="upload-progress" value="0" max="100" style="display: none;"></progress>
        <p id="upload-message" style="display: none;"></p>
    </div>
<!-- Seção para exibir vídeos enviados -->
<section id="uploadedVideos">
    <h3>Vídeos Enviados Recentemente</h3>
    <div id="videoList">
        <!-- Exemplo de vídeo 1 -->
...
// Função para enviar arquivos usando AJAX
uploadForm.addEventListener('submit', function (event) {
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            uploadForm.reset(); // Limpar o formulário
        } else {
            uploadMessage.style.display = 'block';
    ...
    xhr.onerror = function () {
        uploadMessage.style.display = 'block';
        uploadMessage.textContent = 'Erro na conexão. Verifique sua internet.';
        uploadMessage.style.color = 'red';
    };

    xhr.send(formData); // Enviar dados
});
...
document.getElementById('uploadForm').addEventListener('submit', function (e) {
    ...
    xhr.onload = function () {
        ...
        if (xhr.status === 200) {
            ...
            message.style.display = 'block';
            message.textContent = 'Erro no envio do arquivo.';
        }
    };
